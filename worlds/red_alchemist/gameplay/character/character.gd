...
const Inventory = preload("res://worlds/red_alchemist/gameplay/inventory/inventory.gd")
const Gear = preload("res://worlds/red_alchemist/gameplay/gear/gear.gd")
var id: StringName
...
func _init(spawn: Resource, rules: Resource) -> void:
	...
	if not template.is_named:
		template = template.duplicate()
		gender = spawn.gender
		if not spawn.display_name.is_empty():
			template.display_name = spawn.display_name
		var variant: Texture2D = template.gender_sprites.get(gender) as Texture2D
		if variant != null:
			template.sprite = variant
