...
const GearDefinition = preload("res://worlds/red_alchemist/gameplay/gear/gear_definition.gd")
...
func unequip(slot: StringName) -> bool:
	...
	if not inventory.has_space():
		last_error = "Inventory is full. Make space before unequipping."
		return false
